package abby.absent.fluff;

public class Constants {
    public static final String MODID = "absentfluff";

    public static final String GENERAL_GROUP_ID = "general_group";
    public static final String TOOL_GROUP_ID = "tool_group";
    public static final String ACCESSORY_GROUP_ID = "accessory_group";

    public static final String GEM_LORE_TRANSLATION_KEY = "lore.gem.";
    public static final String KEY_TOOLTIP_TRANSLATION_KEY = "item.absentfluff.key_tooltip.";
    public static final String BOUND_TO_POSITION_TOOLTIP_TRANSLATION_KEY = "item.absentfluff.bound_to_position";

    public static final String PICKAXE_SUFFIX = "_pickaxe";
    public static final String AXE_SUFFIX = "_axe";
    public static final String SHOVEL_SUFFIX = "_shovel";
    public static final String SWORD_SUFFIX = "_sword";
    public static final String HOE_SUFFIX = "_hoe";

    public static final String BRACELET_SUFFIX = "_bracelet";
    public static final String NECKLACE_SUFFIX = "_necklace";
    public static final String KEY_SUFFIX = "_key";
    public static final String BOMB_SUFFIX = "_bomb";

    public static final String BLOCK_SUFFIX = "_block";
    public static final String ORE_SUFFIX = "_ore";
    public static final String DEEPSLATE_SUFFIX = "_deepslate";
    public static final String ORE_DEEPSLATE_SUFFIX = Constants.ORE_SUFFIX + Constants.DEEPSLATE_SUFFIX;
    public static final String LAMP_SUFFIX = "_lamp";
    public static final String INVERTED_SUFFIX = "_inverted";
    public static final String INVERTED_LAMP_SUFFIX = LAMP_SUFFIX + INVERTED_SUFFIX;
    public static final String CORE_SUFFIX = "_core";

    public static final String PRIDE_PREFIX = "pride_";
    public static final String CARPET_SUFFIX = "_carpet";
    public static final String ALPACA_WOOL_SUFFIX = "_alpaca_wool" + BLOCK_SUFFIX;
    public static final String FLOWER_WOOL_SUFFIX = "_flower";
    public static final String ALPACA_CARPET_SUFFIX = "_alpaca" + CARPET_SUFFIX + BLOCK_SUFFIX;

    public static final String BRACELET_PATH_PREFIX = "bracelet/";
    public static final String NECKLACE_PATH_PREFIX = "necklace/";
    public static final String KEY_PATH_PREFIX = "key/";
    public static final String PICKAXE_PATH_PREFIX = "pickaxe/";
    public static final String AXE_PATH_PREFIX = "axe/";
    public static final String SHOVEL_PATH_PREFIX = "shovel/";
    public static final String SWORD_PATH_PREFIX = "sword/";
    public static final String HOE_PATH_PREFIX = "hoe/";

    public static final String KUNZITE_OUT_OF_RANGE_TRANSLATION_KEY = "item.absentfluff.kunzite.range";
}
